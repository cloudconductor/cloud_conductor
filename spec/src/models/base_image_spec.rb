...
describe BaseImage do
  before do
    ...
    YAML.stub(:load_file).with(BaseImage::IMAGES_FILE_PATH).and_return('ap-northeast-1' => 'ami-12345678')
  ...
  describe '#name' do
    it 'return string that joined cloud name and operating_system name with hyphen' do
      expect(@base_image.name).to eq("#{@cloud.name}-#{@operating_system.name}")
    end
  end

  describe '#to_json' do
    it 'return valid JSON that is generated from Cloud#template' do
      @base_image.cloud.stub(:template).and_return <<-EOS
        {
          "dummy1": "dummy_value1",
          "dummy2": "dummy_value2"
        }
