...
module CloudConductor
  ...
  describe Route53Client do
    ...
    before do
      AWS.stub!
      resp = route53.client.stub_for(:list_hosted_zones)
      resp.data[:hosted_zones] = [hosted_zone]
      resp = route53.client.stub_for(:list_resource_record_sets)
    ...
    describe '#initialize' do
      it 'store @client when receive access_key and secret_key' do
        ...
        expect(result.client).to be_kind_of AWS::Route53::Client
      end
      it 'raise ArgumentError when do not receive access_key or secret_key' do
        config = { service: 'route53' }
        expect do
          Route53Client.new(config)
        end.to raise_error ArgumentError, 'Need access_key and secret_key to access AWS Route53'
      end
    end

    describe '#update' do
      it 'should success to update Route53 resource record set when receive existant record name' do
        result = route53.update('www.example.com', '127.0.1.1')
        expect(result).to be_truthy
      end
      it 'should success to create Route53 resource record set when receive non-existant record name' do
        result = route53.update('www2.example.com', '127.0.2.1')
        expect(result).to be_truthy
      end
      it 'should fail to update Route53 resource record set when receive non-existant domain name' do
        expect do
          route53.update('www.example.org', '127.0.1.1')
        end.to raise_error RuntimeError, "Cannot find AWS Route53 hosted zone to organize 'example.org'."
      end
    end

    describe '#find_hosted_zone' do
      it 'return hosted zone when receive existant domain name' do
      ...
      it 'return nil when receive non-existant domain name' do
    ...
    describe '#find_resource_record_set' do
      it 'return resource record set when receive existant record name and type' do
      ...
      it 'return nil when receive non-existant record name' do
    ...
    describe '#create_or_update_resource_record_set' do
      ...
      it 'return false when raise error' do
        resp = route53.client.stub_for(:change_resource_record_sets)
    ...
    describe '#log_response' do
      it 'send log message to logger' do
        ...
        response = route53.client.stub_for(:list_hosted_zones)
    ...
    describe '#log_error' do
      it 'send log message to logger' do
        expect(Log).to receive(:error).at_least(:once)
        expect(Log).to receive(:debug).at_least(:once)
