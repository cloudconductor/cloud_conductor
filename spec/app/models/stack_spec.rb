...
describe Stack do
  ...
  before do
    @stack = Stack.new
    @stack.name = 'Test'
    @stack.pattern = pattern_snapshot
    @stack.cloud = cloud
  ...
  describe '.in_progress' do
    it 'returns stacks in progress status' do
      stack = FactoryGirl.create(:stack, environment: environment, pattern: pattern_snapshot, status: :PROGRESS)
  ...
  describe '.created' do
    it 'returns stacks in progress status' do
      stack = FactoryGirl.create(:stack, environment: environment, pattern: pattern_snapshot, status: :CREATE_COMPLETE)
  ...
  describe '#valid?' do
    ...
    it 'return true when name is not unique in two Clouds' do
      FactoryGirl.create(:stack, environment: environment, pattern: pattern_snapshot, name: 'Test', cloud: FactoryGirl.create(:cloud, :openstack))
      expect(@stack.valid?).to be_truthy
    end
    ...
    it 'returns false when pattern status isn\'t CREATE_COMPLETE' do
      @stack.pattern.images << FactoryGirl.create(:image, status: :PROGRESS, pattern_snapshot: @stack.pattern)
      expect(@stack.valid?).to be_falsey
    end
